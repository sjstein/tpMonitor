...
def threaded_client(conn, addr):
    ...
    while True:
        ...
        if str(data.decode('utf-8')).startswith(MSG_READ_ALL):
            echo_stat(f, msg_head + 'READ_ALL request from client : ' + str(addr), INFO)
            try:
                if sensor.read():
                    ...
                    echo_stat(f, msg_head + 'Server sent: ' + data, INFO)
                    conn.send(data.encode())
                    timeout = 2  # Reset retry counter

                else:
                    echo_stat(f, msg_head + 'I2C Sensor read failure, sending err(0),-1,-1 to client', WARN)
                    data = '-1,-1,-1'
                    conn.send(data.encode())
                    echo_stat(f, msg_head + 'Delaying ' + str(timeout) + ' seconds before retry', WARN)
                    time.sleep(timeout)
                    timeout = timeout + 1
            ...
            echo_stat(f, msg_head + 'DISCONNECT requested from client : ' + str(addr), INFO)
            conn.close()
            echo_stat(f, msg_head + 'Client ' + str(addr) + ' : connection closed', INFO)
            return 1
...
echo_stat(f, '|SUPR| Server started : ' + HOST + '(' + str(PORT) + ')', INFO)
...
while True:
    try:
        if sensor.init():
            echo_stat(f, '|SUPR| I2C Sensor initialized', INFO)
            retry = 2  # Reset timer
            break
    except:
        echo_stat(f, '|SUPR| Init sensor failed. Retrying after ' + str(retry) + ' Seconds', INFO)
        time.sleep(retry)
        retry = retry + 1

# We have to read initial values from sensor to update pressure and temperature
if not sensor.read():
    echo_stat(f, '|SUPR| Initial sensor read failed - exiting', INFO)
    exit(1)

echo_stat(f, '|SUPR| Initial Pressure: '
...
echo_stat(f, '|SUPR| Initial Temperature: '
...
# Main loop
while True:
    echo_stat(f, '|SUPR| waiting for client connection...', INFO)
    connection, address = s.accept()     # Wait for connection request from client
    echo_stat(f, '|SUPR| Connection accepted from : ' + str(address), INFO)
    t = threading.Thread(target=threaded_client, args=(connection, address),name='T' + str(tid).zfill(3))
    t.start()
    tid = tid + 1
    if tid > MAXTID:
        tid = 0
    echo_stat(f, '|SUPR| Current number of client threads : {0}'.format(str(threading.activeCount() - 1)), INFO)
