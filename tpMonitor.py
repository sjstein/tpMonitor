...
archiveFreq = 30    # Default archive frequency in seconds
...
parser.add_argument("-f", "--freq", help="Frequency (in secs) to read data [default " + str(archiveFreq) + " sec]",\
                    type=int)
parser.add_argument("-t", "--time", help="Time (in mins) to run [default " + str(runTime) + " min ]", type=int)
...
while True:
    ...
    try:
        ...
        if logging:
            f.close()
            s.close()
        print("Acquisition terminated via user interrupt")
        exit(0)
    finally:
        print("Server reports : " + data + " at " + cTime + "(" + cDate + ")")
        print("Run time       : " + str(accumulatedTime) + " of " + str(int(runTime) * 60) + " seconds")
        print("Current depth  : " + "{0:.2f}".format(float(depth)) + " meters (" + "{0:.2f}".format(depthFt) + " ft)\n")
        if logging:
            f.close()
            s.close()
